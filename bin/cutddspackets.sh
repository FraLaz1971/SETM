#!/bin/bash
## this program to cutddspackets.sh <ddsfile> <npackets>
# gives a file of only <npackets>, handles all packets in a given input dds (binary format) file
# but dumps on stdout only <npackets> (<= totnpackets)
...
if [[ $# != 2 ]]
then
  debug "usage: $(basename $0) <infile.dds> <npackets> [>outfile.dds]"
  debug "set DEBUG=1 to have more info"
  exit $E_BADARGS
else
  [[ $DEBUG == 1 ]] && debug "START OF PROGRAM"
  DEBUG=0 #
  #DUMPCMD="/usr/local/bin/bbe -s -b"
  infile="$1"
  t="$2"
  ...
  do
      ...
      # dump dds header of the current packet
      bbe -s -b $dds_packet_start:$dds_header_length $infile
      # dump the whole ccsds packet
      ...
      DEBUG=1
      [[ $DEBUG == 1 ]] && debug "executing  dds_packet_start=s(($ccsds_packet_start + $dcurrent_packet_length))"
      dds_packet_start=$(($ccsds_packet_start + $dcurrent_packet_length)); #update the offset
...
#
#bin/cutddspackets.sh indata/dds/2019-07-22_tm.dds 20 > indata/dat/2019-07-22_10pkts_tm_$(shortdate).dat
#bin/dds2ccsds.sh indata/dds/2019-07-22_tm.dds 10 > indata/dat/2019-07-22_10pkts_tm_$(shortdate).dat
